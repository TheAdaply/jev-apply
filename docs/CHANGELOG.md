...
- `src/writer/prompts.mjs`, `src/writer/openai.mjs` (`historyFacts`), `src/writer/extract-basic.mjs`
  — résumé reading keeps one `f.education.<slug>` / `f.employment.<slug>` row per degree and role,
  start date in `since:`. The no-model reader keeps degree prose and extracts structured employment
  from explicit work headings and dated role lines, including Present and internship tags; it excludes
  school-leaving records before storing education facts.
- `src/memory/derive.mjs` — `educationHistory()` / `employmentHistory()`, newest first, with parts
  ...
  Distinct dates and subjects are part of the deduplication key, so returning to the same employer
  or earning another same-level degree does not erase history.
...
- `src/plan/repeat.mjs` (new) — the repeater expands into ordinary rows (`education[1].school`), so
  ...
  Current entries do not need end dates when a current checkbox exists; minimum entries ask when
  absent. Entry numbers lead labels so truncated summaries still distinguish them.
- `src/browser/repeat.mjs` (new), `src/plan/execute.mjs` — entry N is made to exist by the
  ...
  ids) each box is found by its label inside its card. School/employer names require normalised
  equality, never prefix/substring or model matching; an unknown Greenhouse school uses Other/check.
  Ashby removes a new or initially empty optional card if its required school cannot commit, preserves the
  query as an ask, and snapshots each card's required controls independently.
  A box whose list the saved words do not match (a Discipline) comes back as a question carrying that
  list, so the answer is one of the form's own entries.
- `eval/plan.test.mjs` — history regressions cover distinct periods/subjects, exact institution
  matching, current entries, minimum cardinality, fallback extraction and synthetic hosted employment
  expansion. Employment remains unverified on a live posting. The original Greenhouse run filled both
  degrees with Other/check; the original Ashby run exposed an invalid optional card, not two-card success.
