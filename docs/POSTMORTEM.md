...
| A3 `conditional_child_filled` | A follow-up textarea holding an explanation under a parent answered **No**, and another under a parent nobody answered | `DEPENDENCY_RE`/`conditionPolarity()` did not list "responded/indicated/chose", and `applyDependencies()` skipped a parent still on `ask` | `dependencyOn()` was only ever run against the two recorded boards' phrasings | `77214e5` · `classes.mjs`, `resolve.mjs` | "dependency: a child whose parent is unanswered is blanked, not filled"; "dependency: a quoted condition token links the child to its parent" + runtime `dependency_child_filled` |
...
assertion in `eval/plan.test.mjs`; B1, B3, B12, B14 and B16 additionally refuse at runtime. What the
...
| B15 `name_pronunciation_as_identity` | The user's **full name** typed into "Name Pronunciation" on a live Affirm board, and the stored pronouns typed into "How do you pronounce your name?" — a personal fact written into a box that asked for neither | `NAME_RULES`' `^names?\b` claims any label starting with "Name", and `SENSITIVE_RE`'s `pronouns?` claimed "pronounce": the row reached `nameRow()` or `pronounRow()` instead of `q.core.name_pronunciation` | a wrong *value* in a correctly-filled-looking box: the field was non-empty, the read-back passed, and no count moved. Seven recorded Affirm postings carry that exact label (docs/research/23-review-pr1.md §0.1) | **in place**: `PRONUNCIATION_RE` (`pronunciation\|phonetic\|\bpronounce`) takes the label out of the name rules and `PRONOUN_ROW_RE` — one exported constant shared by `classify()`, the EEO branch and the evidence tier — stops "pronounce" reading as "pronouns"; the row falls through to `q.core.name_pronunciation` or asks — "guard: B15 name_pronunciation_as_identity — a name-pronunciation row is never filled with the name itself, while a name row still is" + the `Name (phonetic)` and classifier assertions |
| B16 `human_only_submit` | An application submitted by a runner on a board whose Submit is gated by a visible challenge only a person can answer — or, with no such gate, a submit that posts an unsolved challenge and a "submitted" the board never confirmed | `p.auto_submit` was the only question asked before the click; whether the *board* permits an automated one was not modelled (PLAN §2.5 lists Lever as "apply later (visible hCaptcha)") | no graded round used such a board: every Greenhouse and Ashby posting clicks its own Submit, so the class had no instance to fail on | **in place**: an adapter declares `HUMAN_SUBMIT`, `maybeSubmit()` returns before every path that can click and the run settles `ready_to_submit` — "guard: B16 human_only_submit — the board declares a human-only submit step; Greenhouse and Ashby do not". The board's `CONFIRMATION` rules stay unverified until a real submit, so `apply.mjs --resume <slug>` reads them against the tab and reports what they saw — "guard: B16 human_only_submit — a hand-submitted application is verified from the board's own receipt, and the form page is not one" |
